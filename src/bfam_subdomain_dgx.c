...
static int
bfam_subdomain_dgx_put_scalar_fields_p(const char * key, void *val,
    void *arg)
{
...
static void
bfam_subdomain_dgx_get_recv_buffer(bfam_subdomain_t *thisSubdomain,
    void *buffer, size_t recv_sz, void* comm_args)
{
  ...
  if(comm_args == NULL)
  {
    BFAM_ASSERT(recv_sz == data.sub->base.glue_p->fields.num_entries *
    ...
    bfam_dictionary_allprefixed_ptr(&data.sub->base.glue_p->fields, "",
        &bfam_subdomain_dgx_put_scalar_fields_p, &data);
            ...
            for(int j = 0; j < sub->N+1; j++)
              for(int i = 0; i < sub->N+1; i++)
              {
                vn_g_elem[j*(sub->N+1)+i] +=
                  interp1[(sub->N+1)*m+j]*interp2[(sub->N+1)*l+i] * vn_e;
                vp1_g_elem[j*(sub->N+1)+i] +=
                  interp1[(sub->N+1)*m+j]*interp2[(sub->N+1)*l+i] * vp1_e;
                vp2_g_elem[j*(sub->N+1)+i] +=
                  interp1[(sub->N+1)*m+j]*interp2[(sub->N+1)*l+i] * vp2_e;
                vp3_g_elem[j*(sub->N+1)+i] +=
                  interp1[(sub->N+1)*m+j]*interp2[(sub->N+1)*l+i] * vp3_e;
              }
          }
      }
      else BFAM_ABORT("Cannot handle dim = %d",DIM);
    ...
    if(EToHm[k] || glue_m->interpolation[0])
    {
      ...
      if(DIM == 1)
      {
        ...
        for(int j = 0; j < sub_m_Nfp; ++j)
        {
          ...
          bfam_real_t Tp2_e =
            S12_m_elem[fm]*n1[f] + S22_m_elem[fm]*n2[f];
          bfam_real_t Tp3_e =
            S13_m_elem[fm]*n1[f] + S23_m_elem[fm]*n2[f];
            ...
            for(int j = 0; j < sub->N+1; j++)
              for(int i = 0; i < sub->N+1; i++)
              {
                Tn_g_elem[j*(sub->N+1)+i] +=
                  interp1[(sub->N+1)*m+j]*interp2[(sub->N+1)*l+i] * Tn_e;
                Tp1_g_elem[j*(sub->N+1)+i] +=
                  interp1[(sub->N+1)*m+j]*interp2[(sub->N+1)*l+i] * Tp1_e;
                Tp2_g_elem[j*(sub->N+1)+i] +=
                  interp1[(sub->N+1)*m+j]*interp2[(sub->N+1)*l+i] * Tp2_e;
                Tp3_g_elem[j*(sub->N+1)+i] +=
                  interp1[(sub->N+1)*m+j]*interp2[(sub->N+1)*l+i] * Tp3_e;
              }
          }
      }
      else BFAM_ABORT("Cannot handle dim = %d",DIM);
    ...
    void *buffer, size_t send_sz, void *comm_args)
{
  ...
  if(comm_args == NULL)
  {
    ...
    bfam_dictionary_allprefixed_ptr(&data.sub->base.glue_m->fields, "",
        &bfam_subdomain_dgx_get_scalar_fields_m, &data);
  }
  else
  {
    char prefix[BFAM_BUFSIZ];
...
static void
bfam_subdomain_dgx_comm_info(bfam_subdomain_t *thisSubdomain,
    ...
    size_t *send_sz, size_t *recv_sz, void *comm_args)
{
  ...
  *rank   = sub->base.glue_p->rank;
  ...
  if(comm_args != NULL)
  {
    ...
    for(int i = 0; args->face_scalars_m[i] != NULL;i++) count++;
  ...
  *recv_sz  = recv_num*sizeof(bfam_real_t);
  ...
  BFAM_LDEBUG(
      ...
      *rank, (intmax_t) sort[0], (intmax_t) sort[1], (intmax_t) sort[2],
      *send_sz, *recv_sz);
}
                              ...
                              const int                        inDIM)
{
  ...
  for(int i = 0; i < num_interp; ++i)
  {
    projection[i] =
      bfam_malloc_aligned(Nrp*sub_m_Nrp*sizeof(bfam_long_real_t));
    for(int n = 0; n < Nrp*sub_m_Nrp; n++) projection[i][n] = 0;
    for(int n = 0; n < Nrp*sub_m_Nrp; n++) Vt_MP[n] = 0;

    bfam_util_mTmmult(sub_m_Nrp, Nrp, sub_m_Nrp, sub_m_V, sub_m_Nrp,
        massprojection[i], sub_m_Nrp, Vt_MP, sub_m_Nrp);
    bfam_util_mmmult (sub_m_Nrp, Nrp, sub_m_Nrp, sub_m_V, sub_m_Nrp,

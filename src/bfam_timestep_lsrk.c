...
bfam_ts_lsrk_intra_rhs(const char * key, void *val, void *arg)
{
  bfam_ts_lsrk_allprefix_t *data = (bfam_ts_lsrk_allprefix_t *) arg;
  bfam_subdomain_t* sub = (bfam_subdomain_t*) val;
  data->ts->intra_rhs(sub, data->rate_prefix,
...
void
bfam_ts_lsrk_step_extended(bfam_ts_t *a_ts, bfam_long_real_t dt,
      ...
      const char *rate_prefix, const bfam_long_real_t a))
{
  bfam_ts_lsrk_init_extended(ts, dom, method, subdom_match, subdom_tags,
      comm_match, comm_tags, mpicomm, mpitag, comm_data, aux_rates,
      scale_rates,intra_rhs,inter_rhs,add_rates,1);
}

bfam_ts_lsrk_t*
bfam_ts_lsrk_new_extended(bfam_domain_t* dom, bfam_ts_lsrk_method_t method,
    bfam_domain_match_t subdom_match, const char** subdom_tags,
    bfam_domain_match_t comm_match, const char** comm_tags,
    MPI_Comm mpicomm, int mpitag, void *comm_data,
    void (*aux_rates) (bfam_subdomain_t *thisSubdomain, const char *prefix),
    ...
    int make_rates)
{
  ...
  bfam_ts_lsrk_init_extended(newTS, dom, method, subdom_match, subdom_tags,
      comm_match, comm_tags, mpicomm, mpitag, comm_data, aux_rates,
      scale_rates,intra_rhs,inter_rhs,add_rates,make_rates);
  return newTS;
}


void
bfam_ts_lsrk_init_extended(bfam_ts_lsrk_t* ts,
    bfam_domain_t* dom, bfam_ts_lsrk_method_t method,
    bfam_domain_match_t subdom_match, const char** subdom_tags,
    bfam_domain_match_t comm_match, const char** comm_tags,
    MPI_Comm mpicomm, int mpitag, void* comm_data,
    ...
    int make_rates)
{
   ...
   */
   ...
   for(int s = 0; s < numSubs;s++)
   {
     ...
     if(make_rates) aux_rates(subs[s],BFAM_LSKR_PREFIX);
   }
  ...
  switch(method)
  {
    ...
    case BFAM_TS_LSRK_KC54:
      ...
      ts->A[1] = BFAM_LONG_REAL(-567301805773.0)/
                 BFAM_LONG_REAL(1357537059087.0);
      ts->A[2] = BFAM_LONG_REAL(-2404267990393.0)/
                 BFAM_LONG_REAL(2016746695238.0);
      ts->A[3] = BFAM_LONG_REAL(-3550918686646.0)/
                 BFAM_LONG_REAL(2091501179385.0);
      ts->A[4] = BFAM_LONG_REAL(-1275806237668.0)/
                 BFAM_LONG_REAL(842570457699.0);

      ts->B[0] = BFAM_LONG_REAL(1432997174477.0)/
                 BFAM_LONG_REAL(9575080441755.0);
      ts->B[1] = BFAM_LONG_REAL(5161836677717.0)/
                 BFAM_LONG_REAL(13612068292357.0);
      ts->B[2] = BFAM_LONG_REAL(1720146321549.0)/
                 BFAM_LONG_REAL(2090206949498.0);
      ts->B[3] = BFAM_LONG_REAL(3134564353537.0)/
                 BFAM_LONG_REAL(4481467310338.0);
      ts->B[4] = BFAM_LONG_REAL(2277821191437.0)/
                 BFAM_LONG_REAL(14882151754819.0);

      ts->C[0] = BFAM_LONG_REAL(0.0);
      ts->C[1] = BFAM_LONG_REAL(1432997174477.0)/
                 BFAM_LONG_REAL(9575080441755.0);
      ts->C[2] = BFAM_LONG_REAL(2526269341429.0)/
                 BFAM_LONG_REAL(6820363962896.0);
      ts->C[3] = BFAM_LONG_REAL(2006345519317.0)/
                 BFAM_LONG_REAL(3224310063776.0);
      ts->C[4] = BFAM_LONG_REAL(2802321613138.0)/
                 BFAM_LONG_REAL(2924317926251.0);
